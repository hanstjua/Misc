...
int isBSTL(BTNode *root, int max);

int isBSTR(BTNode *root, int min);
...
int isBST(BTNode *root){
  ...
  if(isBSTL(root->left, root->item)==0)return 0;
  if(isBSTR(root->right, root->item)==0)return 0;
...
int isBSTL(BTNode *root, int max){
  ...
  if(root->left != NULL && root->left->item >= root->item)return 0;
  if(root->right != NULL && (root->right->item <= root->item || root->right->item >= max))return 0;
...
int isBSTR(BTNode *root, int min){
  ...
  if(root->left!=NULL && (root->left->item >= root->item || root->left->item <= min))return 0;
  if(root->right!=NULL && root->right->item <= root->item)return 0;
